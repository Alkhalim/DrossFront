...
func _rebuild_production_buttons(building: Building) -> void:
	...
	# Intelligence Network upgrade panel — Meridian-only building with no
	# producible units. Show tier upgrade buttons instead of the empty
	# "No production" fallback.
	var bstats: BuildingStatResource = building.stats as BuildingStatResource
	if bstats != null and bstats.building_id == &"intelligence_network":
...
func _make_intel_upgrade_button(building: Building, title: String, blurb: String, target_tier: int, cost_s: int, cost_f: int) -> Button:
	...
	btn.pressed.connect(_on_intel_upgrade_pressed.bind(building, target_tier, cost_s, cost_f))
	var chip_refs: Dictionary = _attach_cost_widget(btn, cost_s, cost_f, 0)
	_action_buttons.append({ "button": btn, "kind": "intel_upgrade", "cost_s": cost_s, "cost_f": cost_f, "chips": chip_refs })
	return btn
